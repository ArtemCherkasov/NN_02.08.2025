...
public class LSTMCell implements LayerInterface {
    ...
    public double[] sigmaFunction(double[] a) {
        ...
        double[] c = new double[vectorLength];
        for (int vectorIndex = 0; vectorIndex < vectorLength; vectorIndex++) {
            c[vectorIndex] = 1.0 / (1.0 + Math.exp(-1 * a[vectorIndex]));
        }
        return c;
    }
    ...
    public void setExpectedVector(double[] expectedVector) {
        ...
        this.expectedVector = this.tanhFunctionVector(expectedVector);
    }

    public void setTargetSigmaPredictionVector(double[] targetSigmaPredictionVector) {
        this.targetSigmaPredictionVector = targetSigmaPredictionVector;
    }

    public List<Node> getTargetSigmaVector() {
        return targetSigmaVector;
    }
